...
import { Bus, CalendarDays, CircleGauge, Hotel, Soup, UserRound, UserRoundCog } from "lucide-react";
...
export function MobileBottomTabs() {
  ...
  const staffTabHrefs = ["/", "/reservations", "/restaurants", "/hotels"];
  const visibleTabs = auth.role === "admin" ? tabs : tabs.filter((tab) => staffTabHrefs.includes(tab.href));
  return (
    <nav className={cn("fixed inset-x-0 bottom-0 z-40 grid border-t bg-white px-1 py-1 shadow-[0_-10px_30px_rgba(15,23,42,0.08)] lg:hidden", auth.role === "admin" ? "grid-cols-7" : "grid-cols-4")}>
      {visibleTabs.map((tab) => {
        const Icon = tab.icon;
