...
export function MemberTable({ members }: { members: AdminUser[] }) {
  ...
  return (
    <div className="space-y-4">
            ...
            <TableBody>
              {filtered.map((member) => (
                <TableRow key={member.id}>
                  ...
                  <TableCell><StatusBadge value={member.status === "active" ? "활성" : "비활성"} /></TableCell>
                  <TableCell>{member.lastLoginAt || "-"}</TableCell>
                  <TableCell>{member.createdAt}</TableCell>
                  <TableCell><Button size="sm" variant="outline" onClick={() => edit(member)}>수정</Button></TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
      ...
      <MemberForm member={selected} open={open} onOpenChange={setOpen} />
    </div>
  );
}
