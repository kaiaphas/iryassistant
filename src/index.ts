...
async function main() {
  ...
  if (command === "query:schedules") {
    const { data, error } = await supabase
      ...
      .select("id,source_schedule_key,tour_date,tour_type_label,product_name,departure_time,return_time,vehicle_no,vehicle_capacity,guide_name,driver_name,restaurant_names,hotel_name,room_assignments,progress_status_label,notice_memo")
      .eq("is_active", true)
